...
export class RefreshTokenRepository implements IRefreshTokenRepository {
  ...
  async create({ expires, token, user_id, valid }: CreateRefreshTokenDTO): Promise<RefreshToken> {
    const refreshToken = this.repository.create({ expires, token, user_id, valid });
